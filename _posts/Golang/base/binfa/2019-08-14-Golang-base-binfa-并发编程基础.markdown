...
线程的执行总是趋向于CPU受限或I/O受限。换句话说，一些线程需要话费一定时间使用CPU进行计算，而另外一些线程则会话费一些时间等待相对较慢的I/O操作的完成。通常，调度器会依据趋向性猜测，将其分类，并让I/O受限的线程具有更高的动态优先级以有限使用CPU。因为调度器任务I/O操作一般会花费更多的时间，因此应该早点执行。
